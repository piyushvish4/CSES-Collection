...
int f(int i, int w, vector<int> weight, vector<int> value, vector<vector<int>> &dp)
{
	...
	int not_pick=0+f(i-1,w,weight,value,dp);
	int pick=-1e9;
	if(w>=weight[i]) pick=value[i]+f(i-1,w-weight[i],weight,value,dp);
...
void solve(){
    ...
    inp(v1,n);
    inp(v2,n);
    vvi(dp,n,maxweight+1,0);
    rep(i,v1[0],maxweight)dp[0][i]=v2[0];
    loop(i,1,n,1){
        rep(j,0,maxweight){
            int not_pick=dp[i-1][j];
            int pick=INT_MIN;
            if(v1[i]<=maxweight) pick=v2[i]+dp[i-1][maxweight-v1[i]];
