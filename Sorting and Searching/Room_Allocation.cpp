...
void solve(){
    ...
    int cnt=0;
    multimap<int,int>mp;
    multimap<pair<int,int>,int>res;
    rrep(i,n-1,0){
        auto it=mp.upper_bound(v2[i]);
        if(it==mp.end()){
            cnt++;
            mp.insert({v1[i],cnt});
            res.insert({{v1[i],v2[i]},cnt});
        }
        else{
            mp.insert({v1[i],it->second});
            res.insert({{v1[i],v2[i]},it->second});
            mp.erase(it);
        }
    }
    cout<<cnt<<"\n";
    loop(i,0,n,1){
        auto it=res.find({v[i].F,v[i].S});
        cout<<(*it).S<<" ";
        res.erase(it);
    }
}
