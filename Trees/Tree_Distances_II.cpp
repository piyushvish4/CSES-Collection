...
void dfs(int vertex, int parent, int number){
    num[vertex]=number;
    cnt[vertex]=1;
    val[vertex]+=number;
    for(auto child: v[vertex]){
        if(child!=parent){
            dfs(child,vertex,number+1);
            cnt[vertex]+=cnt[child];
            val[vertex]+=val[child];
        }
    }
    res[vertex]=(val[vertex]-cnt[vertex]*num[vertex]);
}

void dfs1(int vertex, int parent){
    if(parent){
        res[vertex]+=res[parent]-res[vertex]-cnt[vertex]+n-cnt[vertex];
    }
    for(auto child: v[vertex]){
